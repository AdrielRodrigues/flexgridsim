 ...
 * @author Helder
 */
public class RCSAMP implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		for(int dm : demands) {
			Path p = getKShortestPath(none, flow.getSource(), flow.getDestination(), dm, false);
			if(p != null) {
				multipath.add(p);
				int[] vet = p.getLinks();
				for(int o = 0; o < vet.length; o++) {
					none.removeEdge(pt.getLink(vet[o]).getSource(), pt.getLink(vet[o]).getDestination());
				}
			}
		}
		
		if(multipath.size() != numMP) {
		...
		for(Path mp : multipath) {
			if (mp.getLinks() == null || mp.getSlotList().isEmpty()) {
				cp.blockFlow(flow.getID());
				return;
			}
		}
		...
		for(Path mp : multipath) {
			Long numb = vt.createLightpath(mp, Modulations.getModulationLevel(pt.getSlotCapacity()));
			if(numb != -1) {
				ids.add(numb);
			} else {
				if(!ids.isEmpty()) {
		...
		for(Path mp : multipath) {
			flow.setLinks(mp.getLinks(), n);
			flow.setSlotList(mp.getSlotList(), n);
			lightpath.add(vt.getLightpath(ids.get(n)));
			n++;
		}
