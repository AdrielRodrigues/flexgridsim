 ...
 * @author Helder
 */
public class RCSA_MG implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		if(path == null) {
			demands = dividir(demandInSlots - guardBand, 2);
			for(int de : demands) {
				Path prov = getKShortestPath(none, flow.getSource(), flow.getDestination(),	de,false);
				int[] d;
				if(prov != null) {
					multipath.add(prov);
					d = prov.getLinks();
					for(int n = 0; n < d.length; n++) {
						none.removeEdge(pt.getLink(d[n]).getSource(), pt.getLink(d[n]).getDestination());
					}
				}
			}
			if(multipath.size() == 2) {
				for(Path m : multipath) {
					if(m.getLinks() == null || m.getSlotList().isEmpty())
						mp = false;
					else
						mp = true;
				}
