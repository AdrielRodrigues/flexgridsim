...
import flexgridsim.util.CopyOfMultiGraph;

/**
 * @author helder
 *
 */
public class BARTRMAN_AP implements RSA  {
	private PhysicalTopology pt;
	private VirtualTopology vt;
	private ControlPlaneForRSA cp;
	private WeightedGraph graph;
	private CopyOfMultiGraph multigraph;
	/**
	 * 
	 */
	...
	public void simulationInterface(Element xml, PhysicalTopology pt,
		...
		this.multigraph = new CopyOfMultiGraph(graph, pt);
						...
						if((pt.getLink(vt.getLightpath(a).getLinks()[0]).getSource()==flow.getSource()&&pt.getLink(vt.getLightpath(a).getLinks()[vt.getLightpath(a).getLinks().length-1]).getDestination()==flow.getDestination())
							...
							if(vt.getLightpath(a).getSlotbyLink()<= demandInSlotsback){
								...
								for(LightPath lp : lightpath) {
									for(Slot s : lp.getSlotList()) {
										st.add(s);
									}
								}
								for(LightPath lp : lightpathp) {
									for(Slot s : lp.getSlotList()) {
										st.add(s);
									}
								}
								...
								for(Slot s : st) {
									multigraph.setAsUsed(pt.getLink(s.link).getSource(), pt.getLink(s.link).getDestination(), s.core, s.slot);
								}
...
public int[] MSP(WeightedGraph G, int s, int demand) {
	...
	public Path getShortestPath(CopyOfMultiGraph multigraph, int src, int dst, int demand) {
		int links[];
		int nowSlot=-1;
		int nowCore=-1;;
		ArrayList<Integer> path = new ArrayList<Integer>();
		...
		for (int j = 0; j < multigraph.getNumMultiedges();j++) {	//Num cores
			for (int i = 0; i < multigraph.getNumEdges() - demand; i++) {	//Num slots left
	...
	public  Path getbestPathBack(int src, int dst, int demand,Path path) {
		Path backupPath;
		CopyOfMultiGraph multigraphb = new CopyOfMultiGraph(graph, pt);		
