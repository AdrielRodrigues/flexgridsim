...
public class EEMPR implements RSA {
    ...
    public void flowArrival(Flow flow) {
        ...
        for (int i = 0; i < menores_caminhos.length; i++) { 
            ...
            //Alterei a forma como a modulação é tratada para ficar fiel ao artigo pois lá ele não é tão conservador
            int modulation = Modulations.getModulationByDistanceHosseini(distance);
    ...
    private double getXtThreshold(int modFormat) {
        // Tabela 1: 16QAM (-31.79), 8QAM (-28.77), QPSK (-25.76), BPSK (muito alto, ex -14 ou ignorável)
        if (modFormat == 3) return -31.79;
        if (modFormat == 2) return -28.77;
        if (modFormat == 1) return -25.76;
