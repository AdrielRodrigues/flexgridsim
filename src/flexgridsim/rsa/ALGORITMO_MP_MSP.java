 ...
 * TwoPaths
 */
public class ALGORITMO_MP_MSP implements RSA {
	...
	public void flowArrival(Flow flow) {
		...
		if (path == null) {
			dm[0] = (int)Math.ceil(((demandInSlots + guardBand) / 2));
			dm[1] = demandInSlots + guardBand - dm[0];
			
			Path prov;
			prov = getKShortestPath(graph, flow.getSource(), flow.getDestination(), dm[0],false);
			...
			if(prov != null) {
				int[] here = prov.getLinks();
				for(int i = 0; i < here.length; i++) {
					newone.removeEdge(pt.getLink(here[i]).getSource(), pt.getLink(here[i]).getDestination());
				}
				
				prov = getKShortestPath(newone, flow.getSource(), flow.getDestination(), dm[1],false);
				if(prov != null)
					mpath.add(prov);
		...
		if(!flag) {
			...
			if(id<0){
				...
				id = vt.createLightpath(mpath.get(0), Modulations.getModulationLevel(pt.getSlotCapacity()));
				long idd = vt.createLightpath(mpath.get(1), Modulations.getModulationLevel(pt.getSlotCapacity()));
				
				if (id >= 0 && idd >= 0) {
					flow.setLinks(mpath.get(0).getLinks(), 0);
					flow.setLinks(mpath.get(1).getLinks(), 1);
					
					flow.setSlotList(mpath.get(0).getSlotList(), 0);
					flow.setSlotList(mpath.get(1).getSlotList(), 1);
					
					lightpath.add(vt.getLightpath(id));
					lightpath.add(vt.getLightpath(idd));
					
					if(!cp.acceptFlow(flow.getID(), lightpath)) {
						cp.blockFlow(flow.getID());
						vt.removeLightPath(id);
						vt.removeLightPath(idd);
					}
 ...
 */
	...
	public ArrayList<Path> getKShortestPath(WeightedGraph G,int src, int dst, int[] demand, boolean overlap){
		...
		for(int t = 0; t < 2; t++) {
			for (int i = 0; i < kPaths.length; i++) {
				if (kPaths[i].length > 1){
					...
					channel=getSimilarSlotsInLinks(links,overlap, demand[t]);
					if(channel!=null){
						paths.add(new Path(links, channel));
						if(paths.size() == 2) {
							return paths;
						}
					}	
