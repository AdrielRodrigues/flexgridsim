...
//    }
    ...
    //	Second
    //
    public boolean canCreateLightpath(ArrayList<Slot> slots, int modulationLevel) {
        try {
        	 for (Slot slot: slots) {
        		 //TODO Maybe develop a more efficient mode to get this link
        		 boolean [][] dispo = pt.getLink(slot.link).getAllocableSpectrum(modulationLevel, ModulationsMuticore.inBandXT[modulationLevel]);
        		 //if (!pt.getLink(slot.link).isSlotsAvailable(slot)) {
        		 if(!dispo[slot.core][slot.slot]) {
        			//Verify the threshold
             		return false;
        		 } else {
